...
def main():
    ...
    parser.add_argument('--archs', nargs='+', required=True, help="List of architectures to run (e.g. resnet10 vgg19_bn for CIFAR, mlp fttransformer for Covertype)")
    ...
    parser.add_argument('--imbalance_factor', type=float, default=5.0, help="CIFAR-10 Minor class ratio (5 to 30).")
    parser.add_argument('--batch_size', type=int, default=None)
    parser.add_argument('--output_dir', type=str, default='./outputs')
    args = parser.parse_args()

    # Pre-emptively set batch sizes depending on dataset if not explicitly set
    ...
    for arch in args.archs:
        ...
        for run_idx in range(args.k_runs):
            ...
            for opt_name in ['AdamW', 'SGD', 'Muon']:
                ...
                run_id = f"Run {run_idx+1}-{opt_name}"
                metrics = train_and_track(model, optimizers, train_ldr, eval_ldr, test_ldr, device, args.epochs, is_cifar, run_id)
                
                # Append metrics to aggregate dict
                for k in metrics:
        ...
        # ---------------------------------------------
        # Plot Averages and Variance per Architecture
        # ---------------------------------------------
        for opt_name in ['AdamW', 'SGD', 'Muon']:
    ...
    # ---------------------------------------------
    # Print and Save Final Accuracy Tables
    # ---------------------------------------------
